...
func TestSelectServiceMatchesCaseInsensitively(t *testing.T) {
	...
	selection, err := selectService(context.Background(), services, "", "ROUTER-01", "SSHD", "192.0.2.10:22", 4, true, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectServiceFiltersByServer(t *testing.T) {
	...
	selection, err := selectService(context.Background(), services, "lab", "router-01", "sshd", "192.0.2.11:22", 4, true, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectServiceRejectsMissingAndAmbiguousMatches(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := selectService(context.Background(), services, "", "router-01", test.serviceName, "", 4, true, nil)
